...
public class User {
	...
	boolean keyVerified;//true when the key has been verified
	...
	public User(String nameIn, String in, String out){//Initialize the user object with values for everything but the password key that needs extra specific method and checks whether the password check reference file exists.
		name = nameIn;
		unencryptedDirectory = new Directory( in);
		encryptedDirectory = new Directory( out);
		referenceFile = new File(encryptedDirectory.getLocation()  + "\\reference." + name);
	}
